...
def parse_args():
    ...
    parser.add_argument('--txt-dir', default='/home/teddy/UCF-QNRF_ECCV18',
                        help='original data directory')
...
# -----------------------------------------------For UCF Dataset-----------------------------------------------#
    ...
    #             with open(os.path.join(args.txt_dir,'{}.txt'.format(sub_phase))) as f:
    #                 for i in f:
    ...
    # for phase in ['Train', 'Test']:
    #     sub_dir = os.path.join(args.origin_dir, phase)
    #     sub_phase_list = ['trainval']
    #     for sub_phase in sub_phase_list:
    #         sub_save_dir = os.path.join(save_dir, sub_phase)
    #         if not os.path.exists(sub_save_dir):
    #             os.makedirs(sub_save_dir)
    #
    #         f = glob(os.path.join(sub_dir, '*.jpg'))
    #         for i in tqdm(f):
    #             im_path = i
    #             name = os.path.basename(im_path)
    #             im, points = generate_data(im_path)
    #             dis = find_dis(points)
    #             points = np.concatenate((points, dis), axis=1)
    #             im_save_path = os.path.join(sub_save_dir, phase+'_'+name)
    #             im.save(im_save_path)
    #             gd_save_path = im_save_path.replace('jpg', 'npy')
    #             np.save(gd_save_path, points)
...
# -----------------------------------------------For ShanghaiTech Dataset-----------------------------------------------#
    ...
    #         im_path = i
    #         name = os.path.basename(im_path)
    #         im, points = generate_sht_data(im_path)
    #         if sub_phase == 'train':
    #             dis = find_dis(points)
    #             points = np.concatenate((points, dis), axis=1)
    #         im_save_path = os.path.join(sub_save_dir, name)
    ...
    for phase in ['train', 'val']:
        sub_dir = os.path.join(args.origin_dir, phase, 'images')
        sub_phase = phase
        sub_save_dir = os.path.join(save_dir, 'trainval')
        if not os.path.exists(sub_save_dir):
            os.makedirs(sub_save_dir)
        ...
        for i in tqdm(f):
            ...
            dis = find_dis(points)
            points = np.concatenate((points, dis), axis=1)
            im_save_path = os.path.join(sub_save_dir, sub_phase+'_'+name)
            im.save(im_save_path)
            gd_save_path = im_save_path.replace('jpg', 'npy')
            np.save(gd_save_path, points)
